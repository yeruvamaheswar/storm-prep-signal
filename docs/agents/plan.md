...
flowchart TD
    ...
    VAL -->|"signal: newest posting rows + current hour"| CR["compute_risk(signal, baseline, margin_pct, lookahead_hours)"]
...
| `load_baseline(path, lookahead_hours=6)` | `storm_prep/baseline.py` | no | Reads `data/baseline_by_lead.json`; raises a named `ValueError` if it is missing or too short. | 1b |
| `compute_risk(signal, baseline, margin_pct=15, lookahead_hours=6)` | `storm_prep/risk.py` | **yes** | Applies the risk rule. Returns a `RiskResult`. | 1, 1b |
...
## Risk rule v2 (Slice 1b, approved 2026-09-25)

`compute_risk(signal, baseline, margin_pct=15, lookahead_hours=6)` is one deterministic, pure
function. `baseline` is the dict from `load_baseline()` (`storm_prep/baseline.py`), which reads
`data/baseline_by_lead.json`. `scripts/make_baseline.py <csv_folder>` rebuilds that file from
saved MIS CSVs.

Why v2: rule v1 compared the next 6 h with the median of the rest of the posting. Scheduled
outages shrink with lead time (forced outages aren't known days ahead), so that median is
structurally low. v1 rated HIGH on 666 of 755 real postings (Aug 25–Sep 25 2026).
...
3. **Window** = the current hour through current hour + `lookahead_hours` − 1 (6 rows by default).
   The lead `L` of a window row is its index: 0 is the current hour.
4. **Typical** = `baseline["median_mw_by_lead"]`: for each lead, the median `hour_total` at that
   lead across past postings.
5. **Peak** = the window row with the largest `hour_total / typical[L]` (the first one on ties).
   It gives `peak_mw`, `peak_hour` and `peak_lead`. `baseline_mw = typical[peak_lead]`.
6. `trigger_mw = baseline_mw * (100 + margin_pct) / 100` (multiply first, so whole numbers stay exact).
...
`load_baseline(path, lookahead_hours)` raises `baseline file missing: <path> (run
scripts/make_baseline.py)` or `baseline too short: N lead hours in <path>, need 6`.

`RiskResult` fields: `level`, `peak_mw`, `peak_hour`, `baseline_mw`, `trigger_mw`,
`margin_mw`, `peak_lead` (added in v2), `driving_zone`, `zone_mw`.

There is **no absolute MW threshold** anywhere in the code or config.

### Fixture numbers, rule v2 (margin 15%, baseline Aug 25–Sep 25 2026, 755 postings)

- Real fixture: peak 22,194 MW at HE15 (lead +2), typical 20,264 MW, trigger 23,303.6 MW,
  margin −1,109.6 MW, so risk **LOW** and mode **NORMAL**.
- Synthetic fixture (+1,500 MW Houston HE16, 2745 → 4245): 23,539 MW at HE16 (lead +3),
  typical 20,229 MW, trigger 23,263.35 MW, so risk **HIGH** and mode **RESERVE**.

### Fixture numbers, rule v1 (historical: step 0 and the baseline decision)
...
```
[RESERVE] risk HIGH | peak outages 19,480 MW at HE14 (next 6 h) vs trigger 19,150 MW (+330 MW; typical for +1 h ahead 16,652 MW over Aug 25-Sep 25 postings +15%) | driving zone: Houston 7,120 MW | data as of 11:00 CT (38 min old) | quality: ok | source: ERCOT NP3-233-CD
```
...
| `stage` | string | `run`, `load_signal`, `fetch_outages`, `validate`, `load_baseline`, `compute_risk`, `decide_mode`, `apply_to_batteries`, `fail_safe` or `prompt` |
