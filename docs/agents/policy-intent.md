...
- Not a change to `allocate` or `discharge` on this slice. Those still sell kilowatts under the floor, or 0 on HOLD. Negative kW are ignored until Rajat implements charge.
...
- `web/src/contracts.ts` is Sunny's. `contracts.py` wins until that file adds the same names.
