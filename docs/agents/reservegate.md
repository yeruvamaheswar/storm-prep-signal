...
```
Files Rajat must not touch: `contracts.py`, `CONSTRAINTS.md`, `policy.py`, `engine.py`, `risk.py`, `signal.py`, `events.py`, `decision.py`, `__main__.py`, `batteries.py`, `requirements.txt`, `.env.example`, anything in `tapes/`, `brief.py`, `screen.py`, `tape.py`, `README.md`, and CI.
...
Merge order: `uma/1b-rule-v2`, then `uma/contracts`, then `rajat/controller` and `sunny/tape-brief` (either order, since they share no files), then `uma/policy`, then `uma/engine` (the tracer), then `rajat/score`, then `sunny/ci-readme`, then `sunny/screen`, then `uma/engine-final`, then `rajat/failures`, then `sunny/demo`.
