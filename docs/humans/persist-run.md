...
After the engine finishes a run it still writes a file on this laptop (`var/runs/<id>.json`). We now also try to copy that file into the `runs` table in Supabase.
